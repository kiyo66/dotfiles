...
# Absolute paths to the repo's config directories (avoid self-referential links)
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_NVIM="${SCRIPT_DIR}/nvim"
REPO_FISH="${SCRIPT_DIR}/fish"

# ensure NVIM_HOME is a directory (avoid tar error if it's a file)
...
make_link() {
  ...
  # Prevent linking a path to itself (defensive)
  if [ "$(readlink -f "${target}")" = "$(readlink -f "${link}")" ] 2>/dev/null; then
    echo "[ERROR] Refusing to link ${link} to itself (${target})." >&2
    return 1
  fi
...
# Link for nvim (use absolute path to repo nvim dir)
make_link "${REPO_NVIM}" "${NVIM_CONFIG}"

# Link for fish (use absolute path to repo fish dir)
if command -v fish >/dev/null 2>&1; then
